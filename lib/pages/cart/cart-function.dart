...
class IsaacCartfunction {
  ...
  addToCart({product, quantity}) async {
    ...
    if (cart == null) {
      ...
      var newCart = [];
      //add product to cart
      newCart.add({"product": product, "quantity": quantity, "product_id" : product
      ["product_id"]});
      //save cart
      ...
      var index = decodedCart.indexWhere((element) =>
       ...
       if (index == -1) {
        //add product to cart
        decodedCart.add({"product": product, "quantity": quantity,
  ...
  removeFromCart({productid}) async {
    ...
    if (cart == null) {
      ...
      decodedCart.removeWhere((element) =>
